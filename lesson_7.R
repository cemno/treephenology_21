...
own_step_model <- function (HourTemp, summ = TRUE){
  return(step_model(HourTemp, 
                    df = data.frame(lower = c(-1000, 1, 2.5, 9, 12.5, 16, 18), 
                                    upper = c(1.5, 2.5, 8.9, 12.5, 16, 18, 1000), 
                                    weight = c(0, 0.5, 1, 0.5, 0, -0.5, -1)),
                    summ = summ))
}
Winters_hours_gaps <- Winters_hours_gaps
own_step_model(Winters_hours_gaps$Temp, summ = TRUE)
...
test <- as.POSIXlt("2009-04-01 11:00", tz = "UTM")
attributes(test)
