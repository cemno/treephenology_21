...
weather<-make_all_day_table(data.frame(Year=c(2002,2002),
                                       Month=c(mon,mon),
...
DM_sensitivity<-ggplot(results,aes(x=Tmin,y=Tmax,fill=CP)) +
  geom_tile() +
  scale_fill_gradientn(colours=alpha(matlab.like(15), alpha = .8),
                       name="Chill/day (CP)") +
...
# apply function for different sites
...
# and now the plots for heat accumulation
